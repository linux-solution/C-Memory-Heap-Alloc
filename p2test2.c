...
int main(int argc, char *argv[])
{
    ...
    if (p5 != NULL)
    {
        printf("2nd p5 should have been NULL, but is %p\n",p5);
        exit(-1);
    }
    //heap_free( (void *)p3 );
...
void *heap_alloc(int num_bytes_to_allocate)
{
    ...
    {
        printf("allocation horbiden.");
        return NULL;
    }
    allocated_size +=num_bytes_to_allocate;
   
    if ((ptr = (char*) malloc(page_num * num_bytes_to_allocate * sizeof(char)) + 8) == NULL)
    {
...
void heap_free(void *allocated_ptr)
{
    free(&allocated_ptr);
}
